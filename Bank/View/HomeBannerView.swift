...
struct HomeBannerView: View {
    ...
    @EnvironmentObject var homeBannerModel: HomeBannerModel
    ...
    var body: some View {
        ...
        let currentOffset = CGFloat(currentIndex) * (homeBannerModel.homeBannerWidth + spacing)
        /// GeometryReader 改变了它显示内容的方式。在 iOS 13.5 中，内容放置方式为 .center。在 iOS 14.0 中则为：.topLeading
        GeometryReader(content: { geometry in
             
            HStack(spacing: spacing){
                 
                ForEach(0..<homeBannerModel.homeBannerCount()){
                    ...
                    Image(homeBannerModel.bannerImage($0)).resizable()
                        /// 自己尝试一下.fill和.fit
                   ...
                   height:geometry.size.height+50,alignment:.leading)
             .offset(x: dragOffset - currentOffset)
             .gesture(dragGesture)
             /// 绑定是否需要动画
             .animation(isAnimation ?.spring():.none)
//             .animation(.spring(), value: self.isAnimation)
             ...
             .onChange(of: currentIndex, perform: { value in
                ...
                if value == 0 {
                    isAnimation.toggle()
                    currentIndex = homeBannerModel.homeBannerCount() - 2
                /// 最后一张的时候currentIndex设置为1关闭动画
                }else if value == homeBannerModel.homeBannerCount() - 1 {
//                    print(geometry.size.height)
        ...
        }).frame(width: homeBannerModel.homeBannerWidth,
                height: homeBannerModel.homeBannerHeight)
    }
}
    ...
    private var dragGesture: some Gesture{
        ...
        DragGesture()
            ...
            .onEnded {
                ...
                /// 防止越界
                currentIndex = max(min(currentIndex, homeBannerModel.homeBannerCount() - 1), 0)
            }
    }
}
